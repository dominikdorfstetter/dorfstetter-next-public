...
const checkAllItemsInGroup = (
  ...
  return groups.every(({ cookie, items }: LegalGroup) => {
    ...
    return items.every(
      ({ cookie }: LegalItem): boolean =>
        cookies[`${groupCookie}`][`${cookie}`] === value,
    );
  });
};
...
function LegalBanner({
  params: { title, cookie, groups, intro, updatedAt },
  translation,
}: Readonly<{
  params: LegalData;
  ...
  const handlePopupClose = useCallback(() => {
    if (!cookies?.[`${cookie}`]) setAllCookies(groups, cookies, cookie, false);
  ...
  return (
    <div
      id={`legal-banner`}
      className={styles.legal__banner}
      aria-hidden={true}
    >
      <div
      ...
      <div className={styles.banner_wrapper}>
        <div className={styles.banner__header}>
          <div className={styles.banner__title} tabIndex={-1}>
            {title}
          </div>
          <div
            tabIndex={0}

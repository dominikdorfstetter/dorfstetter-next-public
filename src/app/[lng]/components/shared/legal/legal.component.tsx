...
async function getLegalData(lng: string): Promise<LegalData | undefined> {
  ...
  try {
    ...
    const apiResponse: LegalObject | LegalObject[] = await fetchData<
      LegalObject | LegalObject[]
    >(apiURL, HTTP_OPTIONS_FORCE_CACHE);
    ...
    if (Array.isArray(apiResponse)) {
      ...
      const localeResponse = await fetchData<LegalObject>(
        localeURL,
...
async function LegalComponent({
  ...
  return (
    <>
      {!legalData ? (
        <div>Error while fetching Legaldata!</div>
      ) : (
        <LegalBanner translation={legalTranslation} params={legalData} />
      )}
    </>
  );
}

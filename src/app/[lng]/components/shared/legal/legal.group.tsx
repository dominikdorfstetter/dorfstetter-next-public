...
import {LegalItem, LegalTranslation} from "@app/[lng]/components/shared/legal/legal.types";
 ...
 */
const allChildrenTrue = (parent: string, group: string, items: LegalItem[], cookies: any): boolean => {
    const parentData = cookies[parent] ?? {};
    const groupData = parentData[group] ?? {};
    const allGroups = items.map(element => element.cookie);

    if(!cookies || !parentData[group] || !items)
        return false;
 ...
 * @constructor
 */
function LegalGroupComponent({params: {title, cookie, items}, parent, group, translation, tabIndex}: Readonly<{
    params: {
        title: string;
        cookie: string;
        items: LegalItem[];
    ...
    const [allItemsTrue, setAllItemsTrue] =
        useState(false);
    ...
    const setAllChildren = useCallback((items: LegalItem[], cookies:  {[p: string]: any}, value: boolean, parent: string) => {
        ...
        <div className={styles.banner__group} key='legal-group' tabIndex={-1}>
            ...
            <label className={styles.banner__checkbox}>
                <input
                    ...
                    onChange={() => setAllChildren(items, cookies[parent], !allItemsTrue, parent)}
                />
                {`${translation.acceptAll}: "${title}"`}
            </label>
            <ul className={styles.banner__group_list}>
                {items?.map(({id, title: itemTitle, content, cookie: itemCookie}: LegalItem) => {
                    ...
                    return <LegalItemComponent params={legalItemOptions} key={`${id}-${group}`} translation={translation} />;
                })}
            </ul>
        </div>
    );

...
type ContentType = 'paragraph' | 'heading' | 'image' | 'list' | 'link' | 'quote' | 'code';
...
const generateContentElement =
    (contentElement: Content, context: string, index: number, translation: {copy2text: string, copy2success: string},
     lng: string = 'de'): React.JSX.Element => {
 ...
 */
const generateCode = (contentElement: Content, index: number, translation: {copy2text: string, copy2success: string}): JSX.Element => {
    const code = contentElement.children?.[0]?.text ?? '';
    const elementId = `code-${index}`;

    return <CodeElement code={code} elementId={elementId} translation={translation} />
}
 ...
 */
const generateQuote = (contentElement: Content, index: number): React.JSX.Element => {
    return <blockquote key={index}>{processElements(contentElement.children)}</blockquote>;
}
 ...
 */
const generateList = (contentElement: Content, index: number): React.JSX.Element => {
            ...
            return <ul key={index}>
                {
                    children?.map((textElement: ContentElement, idx: number) => {
                        return <li key={`li-${idx}`}>{processElements(textElement.children)}</li>;
                    })
                }
            </ul>;
        case 'ordered':
...
const findTranslation = (translations: Translation[], lng: string): string => {
    return translations.find((element: Translation) => element.key === lng)?.value ?? defaultValue;
}
 ...
 */
const generateImage = (contentElement: Content, context: string, lng: string, index: number): React.JSX.Element => {
    ...
    return (
        <picture key={index}>
            ...
            />
            {
                ...
                <figcaption className={`${context}_caption`}>&quot;{getTranslationFromTextElement(caption, lng)}&quot;</figcaption>
            }
        </picture>)
}
 ...
 */
const generateHeading = (contentElement: Content, index: number): React.JSX.Element => {
    const {level} = contentElement;

    switch (level) {
        case 1:
            return <h1 key={index} className={`h1`}>{processElements(contentElement.children)}</h1>;
        case 2:
            return <h2 key={index} className={`h2`}>{processElements(contentElement.children)}</h2>;
        case 3:
            return <h3 key={index} className={`h3`}>{processElements(contentElement.children)}</h3>;
        case 4:
            return <h4 key={index} className={`h4`}>{processElements(contentElement.children)}</h4>;
        case 5:
            return <h5 key={index} className={`h5`}>{processElements(contentElement.children)}</h5>;
        case 6:
            return <h6 key={index} className={`h6`}>{processElements(contentElement.children)}</h6>;
        default:
            return <div key={index} className={`h7`}>{processElements(contentElement.children)}</div>;
    }
}
 ...
 */
const processElements = (elements: ContentElement[] | undefined): React.JSX.Element | React.JSX.Element[] => {
 ...
 */
const generateParagraph = (contentElement: Content, index: number): React.JSX.Element => {
    return <p key={index}>
        {
            contentElement.children?.map((textElement: ContentElement, idx: number) => {
 ...
 */
const generateTextElement = (textElement: ContentElement, idx: number): React.JSX.Element => {
    if (textElement.bold) {
    ...
    return textElement?.text?.length !== 0 ? <span key={idx}>{textElement.text}</span> : <br key={idx}/>;
}
 ...
 */
const generateLink = (contentElement: ContentElement, index: number): React.JSX.Element => {
    const linkText = contentElement?.children?.[0]?.text ?? '';
    return <Link key={index} href={contentElement?.url ?? ''} target="_blank" rel="noreferrer" title={linkText}>
        {linkText}
    </Link>;
}
    ...
    return <div key={index}>
        {
            content?.map((contentElement: Content, index: number) => {
                return generateContentElement(contentElement, context, index, translation);
            })
        }
    </div>;
}

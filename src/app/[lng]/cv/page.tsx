 ...
 */
export default async function CVPage({params}: CVPageProps): Promise<React.JSX.Element> {
    ...
    return (
        <div className={`page_wrapper`}>
            ...
            {cvData && <div className={styles.cv_list}>
 ...
 */
function CVEntryElement({cvEntry, t}: Readonly<{ cvEntry: CVEntry, t: Function }>): React.JSX.Element {
    const skillsExistOnEntry = (cvEntry && cvEntry.skills && cvEntry.skills.length > 0) ?? false;
    ...
    return (
        <div className={styles.card}>
            {cvEntry && <h2 className={styles.cv_position}>{cvEntry?.position ?? t('placeholderPosition')}</h2>}
            {cvEntry &&
                <div className={`${styles.inline_flex}`}>
                    ...
                    {cvEntry?.url && <Link href={cvEntry?.url ?? t('placeholderURL')} target={`_blank`}>
            ...
            {cvEntry &&
                <div className={`${styles.cv_location}, ${styles.inline_flex}`}>
                    <Icon params={iconLocation}/>{cvEntry.location ?? t('placeholderLocation')}
                </div>
            }
            {cvEntry &&
                <div className={`${styles.cv_range}, ${styles.inline_flex}`}>
                    <Icon params={iconMonth}/>
                    {getFormattedDate(fromDate)} - {getFormattedDate(toDate) ?? t('current')}
                </div>
            }
            {skillsExistOnEntry && <div className={styles.cv_skills_wrapper}>
                {cvEntry.skills?.map((entry: Skill) => {
                    return <div key={entry.id}
                                className={styles.cv_list_item}>{entry.title ?? t('placeholderPosition')}</div>;
                })}
            </div>}
        </div>

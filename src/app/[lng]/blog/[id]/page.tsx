...
const POPULATE_QUERY_PARAM = 'populate=coverimage,attachments,photos,links,keywords';
 ...
 */
async function BlogPage({params}: Readonly<{
    params: {
        lng: string;
        id: number;
    }
    ...
    const iconTranslation: LanguageProps = await serverSideTranslation(lng, 'icons');
    ...
    return (
        <div className={styles.blogpage}>
            <div className={styles.cover_container}>
                <Image className={styles.coverimage} src={blog?.coverimage?.data?.attributes?.formats?.large?.url}
            ...
            <div className={styles.content_wrapper}>
                <div className={styles.title}>
                    {blog?.title}
                    <ShareButton lng={lng} url={hostname} title={blog?.title}></ShareButton>
                </div>
                ...
                <ContentPageElement content={content} context={`blog`}
                                    translation={commonTranslation}></ContentPageElement>

                <div className={styles.flex_wrapper}>
                    {
                        attachments.length > 0 ? <AttachmentsList lng={lng} attachments={attachments}/> : <></>
                    }
                </div>
 ...
 * @param id Blog ID
 */
export async function generateMetadata({params: {lng, id}}: Readonly<{
    params: {
        lng: string;
        id: number;
    }

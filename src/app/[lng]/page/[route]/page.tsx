...
import ContentPageElement, {Content} from "@app/[lng]/components/shared/content.element";
import styles from './page.module.scss';
import {Metadata} from "next";
import {HTTP_OPTIONS_CACHE_RELOAD} from "@app/http.headers";
import {Keyword, LocalizationData, PageAPIResponse, PageContent, PageData} from "@app/[lng]/page/[route]/index";
...
async function getPage(lng: string, route: string): Promise<PageData> {
    ...
    const populate: string = 'populate=pageContent,pageContent.context,localizations';
    ...
    try {
        // Get the page data.
        const pageData: PageData[] = await fetchData<PageData[]>(apiURL, HTTP_OPTIONS_CACHE_RELOAD);
 ...
 */
async function CommonPage({params}: Readonly<{
    params: {
        lng: string;
        route: string;
    }
    ...
    return (
        <div className={`page_wrapper`}>
            <ContentPageElement content={content} context={`page`} translation={{copy2success: "", copy2text: ""}}></ContentPageElement>
        </div>
    );
}
 ...
 */
export async function generateMetadata({params: {lng, route}}: {
    params: {
        lng: string;
        route: string;
    }
    ...
    const keywordsCleaned: string[] = keywords?.map((keyword: Keyword) => keyword.keyword) ?? [];
    const metaTitle=
        `${t('meta-common-page-title-prefix')} ${title} ${t('meta-common-page-title-suffix')}`;

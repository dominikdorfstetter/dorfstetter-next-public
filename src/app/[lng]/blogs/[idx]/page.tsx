 ...
 */
async function getBlogs(locale: string, startPage: number): Promise<BlogsAPIResponse> {
    const baseURL = `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/blogs`;
    const fields: string = 'fields[0]=id&fields[1]=title&fields[2]=author&fields[3]=date&fields[4]=teaserText';
    ...
    const apiURL: string =
        `${baseURL}?${fields}&${populate}&locale=${locale}&${pageSize}&${paginationStart}&${sort}`;
 ...
 */
export default async function BlogsPage({params}: Readonly<{
    params: {
        lng: string;
        idx: number;
    }
    ...
    return (
        <div className={styles.blogspage}>
            {showPagination ?
                <h1>{t('headline')} - {t('site')} {currentPage}</h1> :
                <h1>{t('headline')}</h1>
            }
            {
                showPagination &&
                <Pagination lng={lng} route={`blogs`} currentPage={currentPage} pageCount={pageCount}/>
            }
            <br/>

...
const getLandingPageData = async (
  ...
  const apiURL = new URL(
    `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/landing-page`,
  );
...
async function Home({
  ...
  const landingPageData: StartPageAttributes = await getLandingPageData(
    params.lng,
  );
  const { title, introduction, sections, photo } = landingPageData || {};

  const headlineClasses = getClassName(
    styles.gradient_heading,
    styles.gradient_font,
    styles.hero_headline,
  );
  return (
    <main className={`${styles.main} ${styles.page_wrapper}`}>
      ...
      <div className={styles.page_card_wrapper}>
        {sections?.map((entry: Section) => {
          return (
            <SectionElement key={entry.id} entry={entry} lng={params.lng} />
          );
        })}
      </div>
    </main>
 ...
 */
async function SectionElement({
  entry,
  lng,
}: Readonly<{ entry: Section; lng: string }>): Promise<React.JSX.Element> {

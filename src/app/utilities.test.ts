...
describe('fetchData function', () => {
    ...
    it('should throw an error when the response is not ok', async () => {
        ...
        await expect(fetchData('http://mock.api.io', { method: 'GET' })).rejects.toThrow('fake error message');
    });

    it('should throw an error when no data is available', async () => {
        fetchMock.mockResponseOnce(JSON.stringify({}));

        await expect(fetchData('http://mock.api.io', { method: 'GET' })).rejects.toThrow('Received no data from the API.');
    });
});
